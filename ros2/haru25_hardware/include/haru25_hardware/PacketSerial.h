...
//
// https://github.com/bakercp/PacketSerialをBoostベースに書き直した物
// 必要な部分だけ持ってきた
...
class PacketSerial {
   ...
   public:
    ...
    void send(const uint8_t* buffer, size_t size) {
        int numEncoded = cobsEncode(buffer,
                                      size,
                                      encodeBuffer_);
        // 一番最後にパケットマーカーを追加
    ...
    int read(uint8_t* decodeBuffer_) {
        ...
        size_t _receiveBufferIndex = response.size();

        size_t numDecoded = cobsDecode(_receiveBuffer,
                                         _receiveBufferIndex-1, decodeBuffer_);
        return numDecoded;
    }
...
//     float f[127] = {0};
//     packet_.send((uint8_t*)f, 124);
//     auto rx_len = packet_.read(rx_data_);
...
//         min = elapsed;
//     if (++i == 100) {
//         RCLCPP_INFO(this->get_logger(), "rx %ld: %f us: %d us: %d us", rx_len, (float)sum / 100.f, max, min);
//         i = 0;
//         sum = 0;
//         max = 0;
//     }
